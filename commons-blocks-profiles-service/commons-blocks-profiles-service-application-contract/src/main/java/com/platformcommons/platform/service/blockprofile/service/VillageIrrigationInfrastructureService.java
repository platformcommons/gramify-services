package com.platformcommons.platform.service.blockprofile.service;


import com.platformcommons.platform.service.dto.base.PageDTO;
import com.platformcommons.platform.service.blockprofile.dto.*;

import java.util.*;

/**
 * GENERATED_CLASS - DO NOT MODIFY.
 * Service interface for managing VillageIrrigationInfrastructure-related operations.
 * Provides CRUD operations and pagination support for VillageIrrigationInfrastructure entities.
 *
 * @since 1.0.0
 */

public interface VillageIrrigationInfrastructureService {

    /**
     * Retrieves all VillageIrrigationInfrastructures by the given set of IDs.
     *
     * @param ids Set of IDs to fetch
     * @return Set of VillageIrrigationInfrastructureDTO in the same order as retrieved
     */
    Set<VillageIrrigationInfrastructureDTO> getAllByIds(Set<Long> ids);

    /**
     * Creates a new VillageIrrigationInfrastructure entry in the system.
     *
     * @param VillageIrrigationInfrastructure The VillageIrrigationInfrastructure information to be saved
     * @return The saved VillageIrrigationInfrastructure data
     * @since 1.0.0
     */
    VillageIrrigationInfrastructureDTO save(VillageIrrigationInfrastructureDTO VillageIrrigationInfrastructure);

    /**
     * Updates an existing VillageIrrigationInfrastructure entry.
     *
     * @param VillageIrrigationInfrastructure The VillageIrrigationInfrastructure information to be updated
     * @return The updated VillageIrrigationInfrastructure data
     * @since 1.0.0
     */
    VillageIrrigationInfrastructureDTO update(VillageIrrigationInfrastructureDTO VillageIrrigationInfrastructure);

    /**
     * Retrieves a paginated list of VillageIrrigationInfrastructures.
     *
     * @param page Zero-based page index
     * @param size The size of the page to be returned
     * @return A page of VillageIrrigationInfrastructures
     * @since 1.0.0
     */
    PageDTO<VillageIrrigationInfrastructureDTO> getByPage(Integer page, Integer size);

    /**
     * Deletes a VillageIrrigationInfrastructure by their ID with a specified reason.
     *
     * @param id     The ID of the VillageIrrigationInfrastructure to delete
     * @param reason The reason for deletion
     * @return VillageIrrigationInfrastructureDTO
     */
    VillageIrrigationInfrastructureDTO deleteById(Long id, String reason);

    /**
     * Retrieves a VillageIrrigationInfrastructure by their ID.
     *
     * @param id The ID of the VillageIrrigationInfrastructure to retrieve
     * @return The VillageIrrigationInfrastructure with the specified ID
     * @since 1.0.0
     */
    VillageIrrigationInfrastructureDTO getById(Long id);




}
